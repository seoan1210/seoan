...
7. 예외 처리는 \`try/except\`로 처리하고, \`input()\`·무한 루프·파일 접근은 금지합니다.  
8. 필요 시 함수 단위로 모듈화하여 **재사용성과 가독성**을 높이세요.
